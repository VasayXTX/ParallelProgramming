...
struct Block
{
  uint r_low_first, r_low_second;   /*Cols*/
  uint c_low_first, c_low_second;   /*Rows*/
  size_t r_num_first, c_num_second;
  size_t r_c_num;
};
#define BLOCK struct Block

#define N (size_t)pow(10, 1)
#define NN N * N
#define M 2
#define K 3
...
int main(int argc, char *argv[])
{
  ...
  for (l = proc_rank; l < b_muls_num; l += proc_num)
  {
    #ifdef DEBUG
      printf("Mul number: %d\n", l);
      printf("First matrix:: r_low: %d\tc_low: %d\n", b_muls[l].r_low_first, b_muls[l].c_low_first);
      printf("Second matrix:: r_low: %d\tc_low: %d\n", b_muls[l].r_low_second, b_muls[l].c_low_second);
      printf("Nums:: r_num_first: %d\tc_num_second: %d\t\tr_c_num: %d\n\n",
          b_muls[i].r_num_first, b_muls[i].c_num_second, b_muls[i].r_c_num);
    #endif
    for (i = 0; i < b_muls[l].r_num_first; ++i)
    {
      for (j = 0; j < b_muls[l].c_num_second; ++j)
      {
        #ifdef DEBUG
          printf("C[%d,%d] = ", (i + b_muls[l].r_low_first) + 1, j + b_muls[l].c_low_second + 1);
        #endif
        for (k = 0; k < b_muls[l].r_c_num; ++k)
        {
          mat_c[(i + b_muls[l].r_low_first) * N + j + b_muls[l].c_low_second] +=
            mat_a[(i + b_muls[l].r_low_first) * N + k + b_muls[l].c_low_first] * 
            mat_b[(k + b_muls[l].r_low_second) * N + j + b_muls[l].c_low_second];
          #ifdef DEBUG
            printf("A[%d,%d] * B[%d,%d]", 
                (i + b_muls[l].r_low_first) + 1, k + b_muls[l].c_low_first + 1,
                (k + b_muls[l].r_low_second) + 1, j + b_muls[l].c_low_second + 1);
            if (k == b_muls[l].r_c_num - 1) printf("\n"); else printf(" + ");
          #endif
        }
      }
    }
